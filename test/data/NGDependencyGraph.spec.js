...
describe('NGDependencyGraph', function() {
	...
	xit('creates a graph from dependencies', function() {
		var g = new NGDependencyGraph(MockFileObjects.getDependencies());
		expect(g).toBeDefined();
	});

	xit('creates graph nodes from dependencies', function() {
		var g = new NGDependencyGraph(MockFileObjects.getDependencies());
		expect(g.nodes.length).toBeGreaterThan(0);
	});

	xit('supports 1-to-1 and links dependencies from file1 to file3', function() {
		var g = new NGDependencyGraph(MockFileObjects.getDependencies());
	...
	xit('generates 1-to-1 file list', function() {
		var g = new NGDependencyGraph(MockFileObjects.getDependencies());
